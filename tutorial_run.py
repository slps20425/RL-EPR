...
writer = SummaryWriter(
    f'/Users/yi-hsuanlee/Desktop/CSIE/WIDM/Thesis/finRL-elegant/elegant_logs_{nowTime}')
...
class Arguments:
    def __init__(self, agent=None, env=None, if_on_policy=False):
        ...
        self.learning_rate = 2 ** -16  # 2 ** -14 ~= 6e-5
        self.soft_update_tau = 2 ** -8  # 2 ** -8 ~= 5e-3

        if if_on_policy:  # (on-policy)
            self.net_dim = 2 ** 8  # the network width
            # num of transitions sampled from replay buffer.
            self.batch_size = self.net_dim * 4
...
def train_and_evaluate(args, agent_id=0):
    ...
    while if_train:
        ...
        with torch.no_grad():
            if_reach_goal = evaluator.evaluate_and_save(cwd,
    ...
    # temp calculate 20220523 by Evan 
    import pandas as pd 
    import pickle
    ...
    try:
        for _ in range(300):
            tmp_dict= {}
            print(f'{_}times')
            asset_memory, dd_action=eval_env.evan_check_stock_trading_env(args)
        ...
        with open('/Users/yi-hsuanlee/Desktop/WIDM/Thesis/finRL-elegant/dashboard/RLWeight_720d_new_newCritic_bad/720d_200T.pkl', 'wb') as handle:
...
def get_episode_return_and_step(env, act, device) -> (float, int):
   ...
   # import statistics
   # episode_return = statistics.mean(env.portfolio_return_memory) # for Portfolio_cash env only
    return reward, episode_step
