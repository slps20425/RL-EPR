...
def myLogger(name):
    ...
    formatter = logging.Formatter(fmt='%(asctime)s %(levelname)-8s %(message)s',
                                  datefmt='%Y-%m-%d %H:%M:%S')
    handler = logging.FileHandler(f"/Users/yi-hsuanlee/Desktop/CSIE/WIDM/Thesis/finRL-elegant/elegantRL_{nowTime}.log", mode='w')
...
class StockPortfolioCashEnv_AE_30d:
    ...
    def __init__(self, cwd='./envs/FinRL', gamma=0.99,
                 ...
                 ticker_list=None, tech_indicator_list=None, initial_stocks=None, if_eval=False,turbulence_threshold=None,day = 0,if_PCA=False,if_AE_Trend=True,if_additional_data=True,model_type=30):
        ...
        self.data_path = '/Users/yi-hsuanlee/Desktop/WIDM/Thesis/finRL-elegant/dashboard/AE/composite_lstm_45tic_30d_26F@5F_eleganRl.pkl'  #'/Users/yi-hsuanlee/Desktop/WIDM/Thesis/交接的Code和Data/Data/2.5_AE_45_tic/newTill2021/composite_lstm_45tic_30d_26F@5F_eleganRl.pkl' #'/Users/yi-hsuanlee/Desktop/WIDM/Thesis/交接的Code和Data/Data/2.5_AE_45_tic/newTill2021/special/huge_dimension_26F.pkl'#
        data_trend_path = '/Users/yi-hsuanlee/Desktop/WIDM/Thesis/finRL-elegant/dashboard/AE/composite_lstm_45tic_30d_26F@5F_trend_eleganRl.pkl' #'/Users/yi-hsuanlee/Desktop/WIDM/Thesis/交接的Code和Data/Data/2.5_AE_45_tic/newTill2021/newTill2021composite_lstm_45tic_30d_26F@5F_trend_eleganRl.pkl'
        ...
        if not if_eval:
            ...
            df = pd.read_pickle(self.data_path)#pd.read_pickle("/Users/yi-hsuanlee/Desktop/CSIE/WIDM/Thesis/交接的Code和Data/Data/2.5_AE_45_tic/composite_lstm_45tic_30d_26F_5F_eleganRl.pkl")
            df = df.loc['2019-01-04':'2022-03-30']
        self.df = df
        self.priceData = pd.read_pickle('/Users/yi-hsuanlee/Desktop/WIDM/Thesis/finRL-elegant/dashboard/data/latest_45tic_priceBook.pkl') #pd.read_pickle("/Users/yi-hsuanlee/Desktop/WIDM/Thesis/交接的Code和Data/Data/2.5_AE_45_tic/newTill2021/2012-2021_priceBook.pkl")
        ...
        self.day = 0 if self.if_eval else random.randint(0,len(self.totalDay)-1)
        self.amount = None
        ...
        self.max_step = len(self.totalDay)-1-self.model_type
        ...
        if self.if_additional_data:
            self.state = np.hstack((self.portfolio_value*2**-15,self.cash*2**-15,self.actions_memory[-1],self.price*2**-15,self.data_values))
        else:
            self.state = np.hstack((self.portfolio_value*2**-15,self.cash*2**-15,self.actions_memory[-1],self.price*2**-15))
        self.state=self.state.astype(np.float64)
        self.state_dim= len(self.state)
        self.state_memory = [self.state]
        trend = pd.read_pickle(data_trend_path)
        self.if_AE_Trend = if_AE_Trend
        self.trend =trend if self.if_AE_Trend else None
    ...
    def reset(self):
        self.reward_memory = [self.gamma_reward]
        self.asset_memory = [self.initial_amount]
        self.day = random.randint(0,len(self.totalDay)-1-self.model_type) #0 if self.if_eval else random.randint(0,len(self.totalDay)-1)
    ...
    def step(self, actions):
        ...
        self.terminal = (self.day - self.start_day >= self.model_type) or (self.start_day >= len(self.df.index.unique())-1-5 - self.model_type)
        if self.terminal:
            ...
            print(f"Reward {np.mean(self.reward_memory)}")
            ...
            if self.if_additional_data:
                ...
                self.data_values = self.data_values.reshape(self.data_values.shape[0]*self.data_values.shape[1])
                self.state = np.hstack([preprocessing.normalize([portfolio_cash]),preprocessing.normalize([self.price]),[self.actions_memory[-1]],[self.data_values]])[0]
            else:
                self.state = np.hstack([preprocessing.normalize([portfolio_cash]),preprocessing.normalize([self.price]),[self.actions_memory[-1]],[self.data_values]])[0]
            ...
            reward = self.get_reward(self.new_portfolio_return_list,self.actions_memory,trend_action_diff_score) #self.get_reward(self.portfolio_return_memory,self.actions_memory)
            self.reward = reward*self.reward_scaling
        ...
        return self.state, self.reward, self.terminal, trendVSweightValue
    ...
    def get_reward(self,portfolio_return,actions_memory,trend_action_diff_score=0):
        ...
        df_daily_return.columns = ['daily_return']
        actionCount = len(np.unique(actions_memory[1:],axis=0))
        # if df_daily_return['daily_return'].std() !=0:
            #   sharpe = (252**0.5)*df_daily_return['daily_return'].mean()/ \
            #            df_daily_return['daily_return'].std()
            #   print("Sharpe: ",sharpe)
        ...
        # elif self.day == len(self.totalDay)-1:
        #     sharp = np.mean(portfolio_return)/np.std(portfolio_return)
        #     if actionCount < len(actions_memory)//10:
        #         print(f"action less than expected: {self.day//10}")
        #         reward = sharp * (actionCount * 1e-1)
        #     else:
        #         reward = sharp
        #     # if cash less than initial_amount*.1, get penalty
        #     reward = (reward - abs(reward*1e-1)) if self.cash < self.initial_amount*.1 else reward 
        # else:      
        #     sharp = np.mean(portfolio_return)/np.std(portfolio_return)
        #     if actionCount < len(actions_memory)//10:
        #         print(f"action less than expected: {self.day//10}")
        ...
        #     # if cash less than initial_amount*.1, get penalty
        #     reward = (reward - abs(reward*1e-1)) if self.cash < self.initial_amount*.1 else reward 
        #     # if weight diff than predict action and autoencoder trend predict
        #     reward = (reward - abs(reward*1e-1)) if trend_action_diff_score == -1 else reward 
        
            
        #actionSimilarity = dot(actions_memory[-1], actions_memory[-2])/(norm(actions_memory[-1])*norm(actions_memory[-2]))
        return reward
    ...
    def evan_check_stock_trading_env(self,args):
        asset_memory, dd_action =self.evan_draw_cumulative_return(args, torch)
        return asset_memory, dd_action
        
    def evan_draw_cumulative_return(self, args, _torch) -> list:
        ...
        #self.plot(self.date_memory,dd_return,'cumulative return','day','multiple of initial_account',output= f'{cwd}/cumulative_return_portfolio_cash_eval.jpg')
        ...
        # dd_action.to_pickle(f'/Users/yi-hsuanlee/Desktop/WIDM/Thesis/交接的Code和Data/Data/1.5_preprocess_data/new_2021/RL_AE/with_Predictor/@5F/eval_actions_45tic_30d_@{self.data_path[-15:-13]}_{str(datetime.now())}.pkl')
        
        #ploting action
        # self.plot(date,dd_action,'Portfolio_allocation_actions','day','Allocated actions',output= f'{cwd}/eval_portfolio_actions.jpg',legend=dd_action.columns)
        
        # #generate benchmark with actual return
        # import quantstats as qs
        # import time
        # qs.extend_pandas()
        # qs.reports.html(dd_return['daily_returns'], "0050.tw" , output=f'{cwd}/cumulative_return_portfolio_cash_eval_0050tw.html')
        # time.sleep(3)
        # qs.reports.html(dd_return['daily_returns'], "^TWII" , output=f'{cwd}/cumulative_return_portfolio_cash_eval_TWII.html')
